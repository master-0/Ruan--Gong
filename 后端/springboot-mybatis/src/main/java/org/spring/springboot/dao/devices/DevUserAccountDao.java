...
@Mapper
public interface DevUserAccountDao {
    @Select("SELECT * FROM devices where user_account = #{userAccount}")
    // 返回 Map 结果集
    @Results({
    ...
    List<Device> findDeviceByDevUserAccount(@Param("userAccount") String userAccount);
}
